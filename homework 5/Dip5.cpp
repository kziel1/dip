...
int i,j;
Mat gxgx = Mat::zeros(img.rows,img.cols,CV_32FC1);
Mat gygy = Mat::zeros(img.rows,img.cols,CV_32FC1);
Mat gxgy = Mat::zeros(img.rows,img.cols,CV_32FC1);
for(int x=0;x<img.rows;x++){
  		for(int y=0;y<img.cols;y++){
			gxgx.at<float>(x, y)=gradientsX.at<float>(x, y)*gradientsX.at<float>(x, y);
			gygy.at<float>(x, y)=gradientsY.at<float>(x, y)*gradientsY.at<float>(x, y);
			gxgy.at<float>(x, y)=gradientsX.at<float>(x, y)*gradientsY.at<float>(x, y);
  		}
  	}
Mat avggxgx=Mat::zeros(img.rows,img.cols,CV_32FC1);
Mat avggygy=Mat::zeros(img.rows,img.cols,CV_32FC1);
Mat avggxgy=Mat::zeros(img.rows,img.cols,CV_32FC1);

GaussianBlur( gxgx, avggxgx, Size( kernelSize, kernelSize ), sigma);
GaussianBlur( gygy, avggygy, Size( kernelSize, kernelSize ), sigma);
GaussianBlur( gxgy, avggxgy, Size( kernelSize, kernelSize ), sigma);

Mat traces=Mat::zeros(img.rows,img.cols,CV_32FC1);
for(int x=0;x<img.rows;x++){
  		for(int y=0;y<img.cols;y++){
...
Mat determinants=Mat::zeros(img.rows,img.cols,CV_32FC1);
for(int x=0;x<img.rows;x++){
  		for(int y=0;y<img.cols;y++){
...
// Mat weights=Mat::zeros(img.rows,img.cols,CV_32FC1);
// for(int x=0;x<img.rows;x++){
//   		for(int y=0;y<img.cols;y++){
// 			weights.at<float>(x, y)=avggxgx.at<float>(x, y);//*avggygy.at<float>(x, y)-avggxgy.at<float>(x, y)*avggxgy.at<float>(x, y);
// 		}
//   	}
// showImage(determinants, "qwe", 0, true, false);
}
